...
float4 DS8( int2 x )
{
  float4 src1 = DS4( x + uint2( 0, 0 ) );
  float4 src2 = DS4( x + uint2( 0, 4 ) );
  float4 src3 = DS4( x + uint2( 4, 0 ) );
  float4 src4 = DS4( x + uint2( 4, 4 ) );
  return ( src1 + src2 + src3 + src4 ) / 4;
}
...
{
  destination[ dispatchThreadID.xy ] = DS8( dispatchThreadID.xy * 8 );
}
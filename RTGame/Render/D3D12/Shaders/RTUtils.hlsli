                    ...
                    , float tmax )
{
  ...
  HitGeometry procHitGeom;
  procHitGeom.t = tmax + 1;
  ...
  while ( query.Proceed() )
  {
    ...
    {
      subQuery.TraceRayInline( rayTracingScenes[ query.CandidateInstanceID() ], QueryFlags, 0xFF, ray );
      subQuery.Proceed();
      
      [branch]
      if ( subQuery.CommittedStatus() == COMMITTED_TRIANGLE_HIT )
      {
        HitGeometry subHitGeom = CalcHitGeometry( subQuery );
        
        [branch]
        if ( subHitGeom.t < procHitGeom.t )
        {
          procHitGeom = subHitGeom;
          query.CommitProceduralPrimitiveHit( subHitGeom.t );
        }
      }
    }
  }
